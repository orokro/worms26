...
{	
	...
	// skip if leaves are disabled
	if(Match_drawingFlags & DRAW_LEAVES)
	{
		for(i=0; i<5; i++)
		{
	...
	// gtfo if clouds are disabled
	if(Match_drawingFlags & DRAW_CLOUDS)
	{
...
void drawMountains()
{
	// gtfo if mountains are disabled
	if((Match_drawingFlags & DRAW_MOUNTAINS)==FALSE)
		return;
