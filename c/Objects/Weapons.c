...
unsigned long Weapon_props[77] = {
    ...
    // row 1
		...
		// blow torch
        usesRoutine | holdsCustom | noRender | doesntEndTurn | usesPreAim,
        
		// ninja rope
        doesntEndTurn | usesRaycast | holdsCustom | usesAim | usesPreAim,
...
char Weapons_spawn(char type, short x, short y, char xVelocity, char yVelocity, unsigned short time)
{
	...
	if(Weapon_props[(short)type] & usesPhysics)
	{
		...
		new_PhysObj(&Weapon_physObj[slot], &Weapon_x[slot], &Weapon_y[slot], &Weapon_xVelo[slot], &Weapon_yVelo[slot], 70, 100, (char)slot, &Weapon_settled);
...
char Weapons_fire(short charge)
{
	...
	{
		// if it's the NinjaRope, do a raycast to see if we hit land
